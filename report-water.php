...
// error_reporting(E_ALL);
    ...
    $this_url = $_SERVER['PHP_SELF']."?page=report-water" ;
    ...
    if( $_GET['date'] && $_GET['tank_id']){
        ...
        $data = query("SELECT b.description, a.tank_id, a.dailly_use/1000 daily_use, datetime FROM  WWT_act_history a JOIN WWT_act_status b ON b.tank_id=a.tank_id WHERE CONVERT(date, datetime) ='". $_GET['date']."' AND a.tank_id='". $_GET['tank_id'] ."' AND b.description = 'INPUT PDAM' ORDER BY datetime DESC");
    }
    elseif( $_GET['date'] ){
        ...
        WHERE b.description = 'INPUT PDAM' 
        GROUP BY b.description, a.tank_id, CONVERT(VARCHAR(10),datetime,111) ORDER BY CONVERT(VARCHAR(10),datetime,111) DESC");
    }
    // debug($data);die;
...
<h3 class="page-title">Report Water House Monitoring</h3>
</div>
