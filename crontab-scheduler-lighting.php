...
$queryoff = mssql_query("SELECT ip_address,id,Remote_address, datediff(minute, CONVERT(VARCHAR(5),getdate(),108) , time_schedule) as rangeminutes 
                        FROM Lamp_akebono_scheduler WHERE status_schedule = 'off'
...
if($cekoff !=0)
{
    ...
    VALUES('','Automatic Schedule','SCHEDULE LIGHTING','OFF','',
    'Mematikan Lampu sesuai dengan Pengaturan Schedule',
    getdate())
    ");
while($roff = mssql_fetch_assoc($queryoff)){
        $json_data      = file_get_contents("http://".$roff['ip_address'].":1880/lamp/cmd?lamp_id=" . $roff['Remote_address'] . "&status=0");
        $data           = json_decode($json_data);
        mssql_query("UPDATE Lamp_akebono_scheduler SET 
...
$queryoon = mssql_query("SELECT ip_address,id,Remote_address, datediff(minute, CONVERT(VARCHAR(5),getdate(),108) , time_schedule) as rangeminutes 
                        FROM Lamp_akebono_scheduler WHERE status_schedule = 'on'
                        AND datediff(minute, CONVERT(VARCHAR(5),getdate(),108) , time_schedule) between 0 AND 1
                        AND datename(dw,getdate()) IN(
                        SELECT hari FROM Lighting_day WHERE status = 1
                        )
...
if($cekon !=0)
{
    //logs
    mssql_query("INSERT INTO lighting_logs (npk,name,type,status,settime,description,lastupdate)
    VALUES('','Automatic Schedule','SCHEDULE LIGHTING','ON','',
    'Menghidupkan Lampu sesuai dengan Pengaturan Schedule',
    getdate())
    ");

while($ron = mssql_fetch_assoc($queryoon)){
        $json_data_on      = file_get_contents("http://".$ron['ip_address'].":1880/lamp/cmd?lamp_id=" . $ron['Remote_address'] . "&status=1");
